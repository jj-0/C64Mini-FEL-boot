...
{	
	...
	pr_info("sun7i_pcmdma.c::func:%s(line:%d)\n",__func__,__LINE__);
    if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
static int sun7i_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		 ...
		 */
		printk("play DMA_OP_STOP:sun7i-pcmdma.c::func:%s(line:%d)\n",__func__,__LINE__);
...
{	
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (!play_prtd->params) {
			...
			codec_play_dma_conf.dst_drq_type 	= N_DST_IIS1_TX;	
		...
		codec_capture_dma_conf.src_drq_type 	= N_SRC_IIS1_RX;		
...
static int sun7i_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		*/
		...
		printk("func:%s(line:%d)sun7i-pcmdma.c\n",__func__,__LINE__);
			break;
		default:
			ret = -EINVAL;
			break;
